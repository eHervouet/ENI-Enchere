...
public class ArticleEnchereDAOImpl implements ArticleEnchereDAO {
	...
	public List<ArticleEnchere> selectAll() {
		final String SELECT="SELECT nom_article, montant_enchere, date_fin_encheres, pseudo, path_photo "
				+ "FROM Encheres"
		...
		try(Connection cnx = ConnectionProvider.getConnection())
		{
			...
			while(rs.next()) {
				lae.add(new ArticleEnchere(rs.getString("nom_article"), rs.getFloat("montant_enchere"), rs.getDate("date_fin_encheres"), rs.getString("pseudo"), rs.getString("path_photo")));
			}
		} catch(Exception e) {
